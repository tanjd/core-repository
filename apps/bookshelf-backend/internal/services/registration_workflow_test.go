...
func newRegistrationWorkflow() (*RegistrationWorkflow, *repotest.NotificationRepository, *repotest.BookRepository) {
	...
	return NewRegistrationWorkflow(admin, notifs, books, email), notifs, books
}

func TestOnApproved_CreatesInAppNotification(t *testing.T) {
	workflow, notifs, _ := newRegistrationWorkflow()
...
func TestOnRegistered_SkipsWhenEmailDisabled(t *testing.T) {
	workflow, _, _ := newRegistrationWorkflow()
...
func TestCatalogPitch(t *testing.T) {
	t.Run("pluralizes for multiple books", func(t *testing.T) {
		workflow, _, books := newRegistrationWorkflow()
